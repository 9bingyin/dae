/*
 * SPDX-License-Identifier: AGPL-3.0-only
 * Copyright (c) 2022-2025, daeuniverse Organization <dae@v2raya.org>
 */

package sniffing

import (
	"encoding/hex"
	"testing"

	"github.com/daeuniverse/dae/component/sniffing/internal/quicutils"
)

var quicStream, _ = hex.DecodeString("")

func BenchmarkIsLikelyQuicInitial(b *testing.B) {
	for b.Loop() {
		if !IsLikelyQuicInitial(quicStream) {
			b.Fatal("QUIC Initial was rejected")
		}
	}
}

func BenchmarkRejectNonQuicUDP(b *testing.B) {
	packet := []byte("not a QUIC packet")
	for b.Loop() {
		if IsLikelyQuicInitial(packet) {
			b.Fatal("non-QUIC packet was accepted")
		}
	}
}

func BenchmarkQuicSnifferSinglePacket(b *testing.B) {
	for b.Loop() {
		sniffer := NewQuicSniffer()
		result := sniffer.Feed(quicStream)
		_ = sniffer.Close()
		if result.State != QuicSniffFound {
			b.Fatalf("state = %v: %v", result.State, result.Err)
		}
	}
}

func BenchmarkQuicSnifferTwoPackets(b *testing.B) {
	clientHello := testClientHelloWithPadding("example.com", 2200)
	dcid := []byte("benchmark-dcid")
	const split = 1100
	firstPacket := buildTestInitial(b, quicutils.Version_V1, dcid, dcid, nil, 0, 0, clientHello[:split])
	secondPacket := buildTestInitial(b, quicutils.Version_V1, dcid, dcid, nil, 1, split, clientHello[split:])
	b.ResetTimer()

	for b.Loop() {
		sniffer := NewQuicSniffer()
		first := sniffer.Feed(firstPacket)
		result := sniffer.Feed(secondPacket)
		_ = sniffer.Close()
		if first.State != QuicSniffNeedMore || result.State != QuicSniffFound {
			b.Fatalf("states = %v, %v: %v", first.State, result.State, result.Err)
		}
	}
}
